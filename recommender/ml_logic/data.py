...
def read_json(file_path):
    local_playlist_path = os.path.join(os.getcwd(), 'raw_data/playlist_data')
    path = os.path.join(local_playlist_path, file_path)
    print(f"here comes the path {path}")
...
def get_colab_data(slice_amount = 3) -> pd.DataFrame:
    ...
    playlists = glob.glob(f"{local_playlist_path}/mpd.**.json"  )

    playlist_gen =  data_generator(playlists)


    for i, df in enumerate(playlist_gen):
        print(f"File {i} of {len(playlists)}")
...
def main():
    ...
    res = get_track_matrix(df)
    np.save('data/collab_matrix.npy', res)
...
def get_track_matrix(df: pd.DataFrame, normalize=True, chunk_size_mb = 10) -> np.array:
    """
    ...
    #playlist_df["track_id"].to_csv("song_index.csv")
    ...
    return matrix_muliplication(track_to_playlist_matrix)
...
def matrix_muliplication(track_to_playlist_matrix, chunk_size_mb = 10, ):
