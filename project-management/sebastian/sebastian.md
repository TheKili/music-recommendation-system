...
for(let x of dv.page("sebastian").file.inlinks){
	
let text = dv.page(x).file.tasks.text

let tasks = dv.page(x).file.tasks
	.where(t => (t.subtasks == "") ? t.text.includes("[[sebastian") :  (t.text.includes("[[sebastian") || t.subtasks.some(d => d.text.includes("[[sebastian"))
))
